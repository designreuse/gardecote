...
public interface qDocRepository extends PagingAndSortingRepository<qDoc,qDocPK> {
    ...
    @Query("select doc from qDoc doc where ((doc.depart<=:searchDateCapture AND doc.retour>=:searchDateCapture)  ) and ((doc.numImm=:searchBat) or (:searchBat is null) )")
 ...
 //   @Query("select doc from qDoc doc where   (doc.depart between :searchDateCapture1 and :searchDateCapture2 )   and ((doc.numImm=:searchBat) or (:searchBat is null) ) and (doc.enumtypedoc=2)")
  //  public Page<qDoc> findAllMatchedDocsBetweenDatesByConcession(Pageable pageable,@Param("searchDateCapture1") Date searchDateCapture1,@Param("searchDateCapture2") Date searchDateCapture2,@Param("types") List<qTypeConcession> types);
    ...
    @Query("select d from qDebarquement d where (:typeCD member of d.typesConcession) and ((:searchDateCapture1 between  d.depart and d.retour) or (:searchDateCapture2 between d.depart and d.retour) or (d.depart between :searchDateCapture1 and :searchDateCapture2 ) or (d.retour between :searchDateCapture1 and :searchDateCapture2 ) ) ")
    public  List<qDebarquement> findAllMatchedDocsBetweenDatesByConcessionD(@Param("searchDateCapture1") Date searchDateCapture1,@Param("searchDateCapture2") Date searchDateCapture2, @Param("typeCD") qTypeConcession typeCD);

    @Query("select m from qMarree m where ( m.typeConcessionLiee= :typeCD ) and ((:searchDateCapture1 between  m.depart and m.retour) or (:searchDateCapture2 between m.depart and m.retour) or (m.depart between :searchDateCapture1 and :searchDateCapture2 ) or (m.retour between :searchDateCapture1 and :searchDateCapture2 ) ) ")
    public  List<qMarree> findAllMatchedDocsBetweenDatesByConcessionM(@Param("searchDateCapture1") Date searchDateCapture1,@Param("searchDateCapture2") Date searchDateCapture2, @Param("typeCD") qTypeConcession typeCD);

    @Query("select jm from qJourMere jm where ( jm.pageMarree.qmarree.typeConcessionLiee= :tcon ) and ((jm.dateJour between :searchDateCapture1 and:searchDateCapture2)  ) ")
    public List<qJourMere> findAllMatchedJoursM(@Param("searchDateCapture1") Date searchDateCapture1,@Param("searchDateCapture2") Date searchDateCapture2, @Param("tcon") qTypeConcession tcon);

    @Query("select jd from qJourDeb jd where ( :tcon member of jd.pagesDeb.qdebarquement.typesConcession ) OR (jd.dateJour between :searchDateCapture1 and :searchDateCapture2) ")
    public List<qJourDeb> findAllMatchedJoursD(@Param("searchDateCapture1") Date searchDateCapture1,@Param("searchDateCapture2") Date searchDateCapture2,@Param("tcon") qTypeConcession tcon);
